...
class _DashboardScreenState extends State<DashboardScreen> {
  ...
  String _resolveReporter(Map<String, dynamic> data, Map<String, Map<String, dynamic>> citizenByUid) {
    final uid = (data['user_id'] ?? data['userId'] ?? '').toString();
    final citizen = uid.isNotEmpty ? citizenByUid[uid] : null;
    final cName = citizen == null ? '' : (citizen['name'] ?? '').toString();
    final cUsername = citizen == null ? '' : (citizen['username'] ?? '').toString();
    final candidates = [
      data['user_name'],
      data['userName'],
      data['citizen'],
              ...
              child: latest == null
                  ...
                  : Column(
                      ...
                      children: [
                        ...
                        const SizedBox(height: 8),
                        Text('time: ${_formatTimestamp(latest['created_at'] ?? latest['timestamp'] ?? latest['time'])}', style: const TextStyle(color: Colors.white, fontSize: 20, fontWeight: FontWeight.w500)),
                        const SizedBox(height: 8),
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          const SizedBox(height: 8),
          Text('Reporter: ${incident['user_name'] ?? incident['name'] ?? incident['username'] ?? incident['userId'] ?? incident['user_id'] ?? 'Unknown'}', style: const TextStyle(fontWeight: FontWeight.bold, fontSize: 16)),
          const SizedBox(height: 8),
          Text('address: ${incident['address'] ?? incident['location'] ?? 'Unknown'}', style: TextStyle(color: Colors.grey[700], fontSize: 14)),
          const SizedBox(height: 4),
          Text('time: ${_formatTimestamp(incident['created_at'] ?? incident['timestamp'] ?? incident['time'])}', style: TextStyle(color: Colors.grey[700], fontSize: 14)),
          const SizedBox(height: 12),
      ...
      builder: (context, constraints) {
        ...
        return StreamBuilder<List<Map<String, dynamic>>>(
          ...
          builder: (context, citizenSnapshot) {
            final citizens = citizenSnapshot.data ?? [];
            final Map<String, Map<String, dynamic>> citizenByUid = {};
            ...
            return StreamBuilder<List<Map<String, dynamic>>>(
              ...
              builder: (context, snapshot) {
                ...
                sorted.sort((a, b) {
                  final da = a;
                  final db = b;
                  final sa = (da['severity'] ?? 'low').toString().trim().toLowerCase();
                  final sb = (db['severity'] ?? 'low').toString().trim().toLowerCase();
                  ...
                  final ta = da['created_at'] ?? da['timestamp'] ?? da['time'];
                  final tb = db['created_at'] ?? db['timestamp'] ?? db['time'];
                  final ma = ta is String ? DateTime.parse(ta).millisecondsSinceEpoch : 0;
                  final mb = tb is String ? DateTime.parse(tb).millisecondsSinceEpoch : 0;
                  return mb.compareTo(ma);
                });
                              ...
                              Expanded(
                                child: GridView.builder(
                                  ...
                                  itemBuilder: (context, index) {
                                    ...
                                    final id = (data['id'] ?? '').toString();
                                    final uid = (data['user_id'] ?? data['userId'] ?? '').toString();
                                    final reporter = _resolveReporter(data, citizenByUid);
                                    ...
                                    return Container(
                                      ...
                                      child: Column(
                                        ...
                                        children: [
                                          ...
                                          Text(
                                            'time: ${_formatTimestamp(data['created_at'] ?? data['timestamp'] ?? data['time'])}',
                                            style: const TextStyle(color: Colors.white, fontSize: 12),
                                          ),
                                          const SizedBox(height: 8),
                                          Builder(builder: (context) {
